defmodule ChatelWeb.CoreComponents do
  @moduledoc """
  ...
  use Gettext, backend: ChatelWeb.Gettext
  ...
  def user_chat_button(assigns) do
    ~H"""
    <.link
      class={[
        @class
  ...
  def group_chat_button(assigns) do
    ~H"""
    <.link
      class={[
        @class
  ...
  def message_input(assigns) do
    ...
    <div class="p-4 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700">
      <%= if @current_chat do %>
        ...
        >
          <.input
            class="w-full rounded-full p-3 bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
            id="chat-input"
            name="message"
            type="text"
  ...
  defp format_time(time) do
    ...
    time = NaiveDateTime.add(time, 5 * 3600)
    hour = Integer.to_string(time.hour) |> String.pad_leading(2, "0")
    minute = Integer.to_string(time.minute) |> String.pad_leading(2, "0")
    "#{hour}:#{minute}"
  end
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(ChatelWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(ChatelWeb.Gettext, "errors", msg, opts)
    end
  end
