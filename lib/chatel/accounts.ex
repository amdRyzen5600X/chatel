defmodule Chatel.Accounts do
  ...
  alias Chatel.Accounts.{User, UserToken, UserNotifier}
  ...
  def get_user_by_login_and_password(login, password)
    ...
    if Regex.match?(~r/^[^\s]+@[^\s]+$/, login) do
      user = Repo.get_by(User, email: login)
      if User.valid_password?(user, password), do: user
    else
      user = Repo.get_by(User, username: login)
      if User.valid_password?(user, password), do: user
    end
  end
  ...
  def update_user_username(user, token) do
    ...
    with {:ok, query} <- UserToken.verify_change_username_token_query(token, context),
         %UserToken{username: username} <- Repo.one(query),
